...
def get_brats2021_datalist(root_dir, latent_dir):
    """
...
def get_brats_dataloader(root_dir="~/hao/data/brats/ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData",
    ...
    # Get datalist
    datalist = get_brats2021_datalist(root_dir,latent_dir)
    if mode == 'train':
    ...
    # print("k for k in key_to_load if k != latent]=", [k for k in key_to_load if k != "latent"])
    ...
    transforms = Compose([
        ...
        CropAroundPositiveRegiond(
            keys=[k for k in key_to_load if k != "latent" and k != "broken-latent"],  # Exclude latent and mask from cropping
            label_key="mask",   # Automatically add in this
            spatial_size=spatial_size,
        ),


        Lambdad(keys="latent", func=lambda x: np.load(x)["data"].astype(np.float32)),  # Load latent representation
        Lambdad(keys="broken-latent", func=lambda x: np.load(x)["data"].astype(np.float32)),

        # Augmentation
        # RandDropoutd(keys=["latent"], prob=0.2, dropout_prob=0.1)
